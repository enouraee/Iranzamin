...
from .models import REQUEST_STATUS_CHOICES, REQUEST_TYPE_CHOICES
from .selectors import request_get, request_list, request_matches
from .services import request_create, request_delete, request_mark_done, request_update
...
def _matched_property_data(prop):
    if prop is None:
        return None
    return {"id": prop.pk, "address": prop.address}


class _RequestOutputMixin(serializers.Serializer):
    id = serializers.IntegerField()
    customer = serializers.SerializerMethodField()
    region = serializers.SerializerMethodField()
    matched_property = serializers.SerializerMethodField()
    request_type = serializers.CharField()
    status = serializers.CharField()
    target_property_type = serializers.CharField(allow_null=True)
    units_count = serializers.IntegerField(allow_null=True)
    ...
    wants_parking = serializers.BooleanField()
    wants_elevator = serializers.BooleanField()
    wants_storage = serializers.BooleanField()
    ...
    def get_matched_property(self, obj):
        return _matched_property_data(obj.matched_property)


class RequestListApi(APIView):
    permission_classes = [IsAuthenticated]

    class OutputSerializer(_RequestOutputMixin):
        pass
...
class RequestCreateApi(APIView):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        target_property_type = serializers.CharField(required=False, allow_null=True, allow_blank=True)
        units_count = serializers.IntegerField(required=False, allow_null=True, min_value=1)
        ...
        wants_parking = serializers.BooleanField(required=False, default=False)
        wants_elevator = serializers.BooleanField(required=False, default=False)
        wants_storage = serializers.BooleanField(required=False, default=False)
    ...
    def post(self, request: DRFRequest) -> Response:
        ...
        req = request_create(
            ...
            target_property_type=data.get("target_property_type"),
            ...
            wants_parking=data.get("wants_parking", False),
            wants_elevator=data.get("wants_elevator", False),
            wants_storage=data.get("wants_storage", False),
...
class RequestUpdateApi(APIView):
    ...
    class InputSerializer(serializers.Serializer):
        ...
        min_build_year = serializers.IntegerField(required=False, allow_null=True, min_value=1300)
        max_build_year = serializers.IntegerField(required=False, allow_null=True, min_value=1300)
        wants_parking = serializers.BooleanField(required=False)
        wants_elevator = serializers.BooleanField(required=False)
        wants_storage = serializers.BooleanField(required=False)
        max_deposit = serializers.IntegerField(required=False, allow_null=True, min_value=1)
        max_rent = serializers.IntegerField(required=False, allow_null=True, min_value=1)
        budget = serializers.IntegerField(required=False, allow_null=True, min_value=1)
        deadline = serializers.DateField(required=False, allow_null=True)
...
class RequestMarkDoneApi(APIView):
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request: DRFRequest, request_id: int) -> Response:
        serializer = self.InputSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        req = request_mark_done(
