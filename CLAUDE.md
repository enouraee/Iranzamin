...
- **Backend**: Django + DRF in `backend/`, strict **HackSoft Styleguide** (services/selectors/thin apis), packaged with **uv**, on **MySQL via PyMySQL** (`iranzamin` @ `127.0.0.1`, `root`/`abcd@1234`).
- **Code style**: simple, optimized, human-readable; follow **DRY** + **SOLID**. Clarity over cleverness.
